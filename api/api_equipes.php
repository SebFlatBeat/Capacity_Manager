...
if ($res_equipes) {
    while ($e = pg_fetch_assoc($res_equipes)) {
        ...
        $res_membres = pg_query($dbconn, "SELECT m.id_membre, m.nom, m.role, p.nom_pays 
            FROM membres m 
            LEFT JOIN pays p ON m.id_pays = p.id_pays 
            WHERE m.id_equipe = '$tid' 
               OR m.id_equipe LIKE '$tid,%' 
               OR m.id_equipe LIKE '%,$tid' 
               OR m.id_equipe LIKE '%,$tid,%'
...
if (!$data && $method === 'POST') { echo json_encode(array("status" => "error", "message" => "Données invalides.")); exit; }
...
if ($method === 'POST') {
    ...
    if ($action === 'update_team_metrics') {
        ...
        $check = pg_query($dbconn, "SELECT id_ratio FROM kpi_ratios_equipe WHERE id_equipe = $id_equipe AND pi_code = '$pi'");
        if ($check && pg_num_rows($check) > 0) {
            $query = "UPDATE kpi_ratios_equipe SET charge_engagee = $charge, realise = $realise, point_dev_jour = $vel WHERE id_equipe = $id_equipe AND pi_code = '$pi'";
        } else {
        ...
        $check = pg_query($dbconn, "SELECT id_membre FROM absences_sprints WHERE id_membre = $id_m AND pi_code = '$pi' AND numero_iteration = $iter");
        if ($check && pg_num_rows($check) > 0) {
            $query = "UPDATE absences_sprints SET jours_conges = $abs WHERE id_membre = $id_m AND pi_code = '$pi' AND numero_iteration = $iter";
        } else {
